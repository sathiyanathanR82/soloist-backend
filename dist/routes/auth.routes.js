...
// Yahoo OAuth
router.get('/yahoo', passport_1.default.authenticate('yahoo', { scope: ['openid', 'profile', 'email'] }));
router.get('/yahoo/callback', passport_1.default.authenticate('yahoo', { failureRedirect: '/' }), (req, res) => {
    const { token, isNewUser } = req.user;
    const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:4200';
    res.redirect(`${frontendUrl}/profile?token=${token}&isNewUser=${isNewUser}`);
});
